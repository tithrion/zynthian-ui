...
class zynthian_gui_arranger(zynthian_gui_base.zynthian_gui_base):
    ...
    def __init__(self):
        ...
        # Quantity of rows (tracks) displayed in grid
        self.vertical_zoom = self.zynseq.libseq.getVerticalZoom()
        # Quantity of columns (time divisions) displayed in grid
        ...
        # Location of selected cell (time div, row)
        self.selected_cell = [0, 0]
        self.pattern = 1  # Index of current pattern to add to sequence
        # Index of pattern to actually add (may be copied / moved pattern)
        self.pattern_to_add = 1
        ...
        # 2D array of cells 0:cell, 1:cell label
        self.cells = [
            [None] * 2 for _ in range(self.vertical_zoom * self.horizontal_zoom)]
        ...
        self.sequence_title_canvas.bind(
            "<ButtonPress-1>", self.on_sequence_drag_start)
        self.sequence_title_canvas.bind(
            "<ButtonRelease-1>", self.on_sequence_drag_end)
        self.sequence_title_canvas.bind(
            "<B1-Motion>", self.on_sequence_drag_motion)
        ...
        self.timebase_track_canvas = tkinter.Canvas(self.main_frame,
                                                    ...
                                                    bd=0,
                                                    highlightthickness=0)
        self.timebase_track_canvas.bind(
            "<ButtonPress-1>", self.on_time_drag_start)
        self.timebase_track_canvas.bind(
            "<ButtonRelease-1>", self.on_time_drag_end)
        self.timebase_track_canvas.bind(
            "<B1-Motion>", self.on_time_drag_motion)
    ...
    def menu_cb(self, option, params):
        if params == 'Tempo':
            ...
            self.enable_param_editor(self, 'hzoom', {
    ...
    def clear_sequence(self, params=None):
        ...
        self.zyngui.show_confirm(
            f"Clear all tracks and patterns from sequence '{name}'?", self.do_clear_sequence)
    ...
    def remove_track(self, params=None):
        self.zyngui.show_confirm("Remove track {} from sequence {}?".format(
            self.track + 1, self.sequence + 1), self.do_remove_track)
    ...
    def select_smf(self, params=None):
        ...
        for i in file_list:
            options[i[4]] = i[0]
        self.zyngui.screens['option'].config(
            "Import SMF", options, self.smf_file_cb)
        self.zyngui.show_screen('option')
    ...
    def do_import_smf(self, fpath):
        ...
        # Array of [pattern,time,velocity,step] indicating time that note on event received for matching note off and deriving duration
        note_on_info = [[[None, None, None, None]
                         for i in range(127)] for i in range(16)]
        # Position of current pattern within track in ticks
        pattern_position = [self.selected_cell[0]
                            * ticks_per_beat for i in range(16)]
        ...
        while zynsmf.libsmf.getEvent(smf, True):
            ...
            if type == 0x01:
                ...
                if status in note_on and velocity:
                    ...
                    note_on_info[channel][note] = [
                        pattern[channel], time, velocity, step]
                    self.zynseq.libseq.selectPattern(pattern[channel])
                    # Add short event, may overwrite later when note-off detected
                    self.zynseq.libseq.addNote(step, note, velocity, 1, 0)
    ...
    def set_pattern(self, pattern):
        ...
        self.pattern_canvas.itemconfig(
            "patternIndicator", text=f"{self.pattern}")
        ...
        if event.num == 4:
            ...
            if self.row_offset:
                ...
                if self.selected_cell[1] >= self.row_offset + self.vertical_zoom:
                    self.select_cell(
                        self.selected_cell[0], self.row_offset + self.vertical_zoom - 1)
                self.redraw_pending = 4
    ...
    def on_time_drag_motion(self, event):
        ...
        duration = int(self.zynseq.libseq.getPatternLength(
            self.pattern_to_add) / self.clocks_per_division)
        if self.selected_cell[0] < self.col_offset:
            ...
            self.select_cell(
                self.col_offset + self.horizontal_zoom - duration, self.selected_cell[1])
        else:
            self.select_cell()
    ...
    def on_grid_press(self, event):
        ...
        self.select_cell(self.col_offset + int(c),
                         self.row_offset + int(r), False)
    ...
    def on_grid_release(self, event):
        ...
        if self.add_event(self.selected_cell[0], self.sequence, self.track):
            if self.source_col != self.selected_cell[0] or self.source_row != self.selected_cell[1]:
                self.remove_event(
                    self.source_col, self.source_seq, self.source_track)
                self.draw_row(self.source_seq - self.row_offset)
        else:
            # Failed to add pattern so reselect original cells
            self.select_cell(self.source_col, self.source_row)
        self.pattern_to_add = self.pattern
    ...
    def on_grid_drag(self, event):
        ...
        if col != self.selected_cell[0] or row != self.selected_cell[1]:
            # Move selection to show wireframe of moving (dragging) pattern
            self.select_cell(col, row, False, False)
    ...
    def draw_sequence_label(self, row):
        ...
        self.sequence_title_canvas.create_text((0, self.row_height * (row + 1) - 1),
                                               font=tkfont.Font(
                                                   family=zynthian_gui_config.font_topbar[0], size=int(self.fontsize * 0.9)),
                                               fill=CELL_FOREGROUND, tags=(f"rowtitle:{row}", "sequence_title"), anchor="sw",
                                               text=f"{track_name}")
        self.sequence_title_canvas.tag_bind(
            'sequence_title', "<Button-1>", self.on_sequence_click)
    ...
    def draw_cell(self, col, row):
        ...
        cell_index = row * self.horizontal_zoom + col
        if cell_index >= len(self.cells):
            return

        sequence = self.sequence_tracks[row + self.row_offset][0]
        track = self.sequence_tracks[row + self.row_offset][1]
        time = (self.col_offset + col) * \
        ...
        if not cell:
            ...
            cell = self.grid_canvas.create_rectangle(
                coord, fill=fill, width=0, tags=(f"{col},{row}", "gridcell", "pattern"))
            celltext = self.grid_canvas.create_text(
                coord[0] + 1, coord[1] + self.row_height / 2, fill=CELL_FOREGROUND, tags=(f"{col},{row}", "pattern"))
            self.grid_canvas.tag_bind(
                "pattern", '<ButtonPress-1>', self.on_grid_press)
            self.grid_canvas.tag_bind(
                "pattern", '<ButtonRelease-1>', self.on_grid_release)
            self.grid_canvas.tag_bind(
                "pattern", '<B1-Motion>', self.on_grid_drag)
            # Assume cells are always created left to right
            self.grid_canvas.tag_lower(cell)
        ...
        if pattern == -1:
            ...
            self.grid_canvas.itemconfig(celltext, text=pattern, state='normal', font=tkfont.Font(
                family=zynthian_gui_config.font_topbar[0], size=self.fontsize))
            self.grid_canvas.coords(
                celltext, coord[0] + int(duration * self.column_width / 2), int(coord[1] + self.row_height / 2))
    ...
    def draw_grid(self):
        if self.redraw_pending == 1:
            ...
            self.draw_cell(
                self.selected_cell[0] - self.col_offset, self.selected_cell[1] - self.row_offset)
            return
    ...
    def select_cell(self, time=None, row=None, snap=True, scroll=True):
        ...
        if snap:
            ...
            for previous in range(time - 1, -1, -1):
                ...
                prev_duration = int(self.zynseq.libseq.getPatternLength(
                    prev_pattern) / self.clocks_per_division)
                prev_start = previous
        ...
        coord = self.get_cell_coord(
            time - self.col_offset, row - self.row_offset, duration)
        coord[0] = coord[0] - 1
        coord[1] = coord[1] - 1
        coord[2] = coord[2]
        coord[3] = coord[3]
    ...
    def refresh_status(self):
        ...
        for row in range(self.vertical_zoom):
            ...
            if sequence != previous_sequence and sequence != -1:
                self.grid_canvas.coords(
                    f"playheadline-{seq_row}", x, y1, x, y2)
    ...
    def zynpot_cb(self, i, dval):
        ...
        if i == self.ctrl_order[0] and zynthian_gui_config.transport_clock_source <= 1:
            ...
            self.select_cell(
                self.selected_cell[0], self.selected_cell[1] + dval)
        elif i == self.ctrl_order[3]:
            # Time-axis selection
            self.select_cell(
                self.selected_cell[0] + dval, self.selected_cell[1])
